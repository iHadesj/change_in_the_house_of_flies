...
function App() {
  ...
  const swarm = useCallback(
    (x, y) => {
      ...
      swarmTimersRef.current.calm = window.setTimeout(
        () => setInfested(false),
        1900,
      );
    },
  ...
  const cursorIndex = Math.min(
    syncCursor,
    Math.max(0, lyrics.lines.length - 1),
  );
  const activeSection =
    activeLine >= 0 ? (lyrics.lines[activeLine]?.section ?? null) : null;
  ...
  const lyricsStatus = (() => {
    ...
    if (!syncedCount)
      return `${lyrics.lines.length} LINHAS / SEM SYNC${suffix}`;
  ...
  const stampLine = useCallback(() => {
    ...
    commitLyrics(
      lyrics.lines.map((line, i) =>
        i === index ? { ...line, time: audio.currentTime } : line,
      ),
      lyrics.meta,
    );
  ...
  const tagSection = useCallback(
    (index, section) => {
      ...
      commitLyrics(
        lyrics.lines.map((line, i) =>
          i >= start && i < end ? { ...line, section } : line,
        ),
        lyrics.meta,
      );
    },
  ...
  const tagTwinBlocks = useCallback(
    (index) => {
      ...
      blocks.forEach((block) => {
        if (blockSignature(lyrics.lines, block.start, block.end) !== signature)
          return;
      ...
      commitLyrics(
        lyrics.lines.map((line, i) =>
          painted.has(i) ? { ...line, section: origin.section } : line,
        ),
        lyrics.meta,
      );
    },
  ...
  const exportLrc = useCallback(() => {
    const blob = new Blob([toLrc(lyrics)], {
  ...
  return (
    ...
    >
            ...
            >
              ...
              <div className="meaning-scroll">
                ...
                {MEANING.map((entry) => (
                  <section
                    key={entry.n}
                    className={entry.summary ? "meaning-summary" : undefined}
                  >
                    <span>{entry.n}</span>
                    <div>
                      <h3>{entry.title}</h3>
            ...
            >
              ...
              {hasLyrics ? (
                ...
                >
                  <ol>
                    {lyrics.lines.map((line, index) => {
                      const startsSection =
                        line.section &&
                        line.section !== lyrics.lines[index - 1]?.section;
                      ...
                      return (
                        <Fragment key={`${index}-${line.text}`}>
                          {startsSection && (
                            <li
                              className={`lyric-section is-${line.section}`}
                              aria-hidden="true"
                            >
                ...
                <div className="lyrics-scroll lyrics-empty">
                  <p>
                    Nenhuma letra carregada — o projeto não distribui letra
                    nenhuma. Cole a sua abaixo (uma linha por linha) ou deixe um
                    arquivo em <code>public/lyrics/change.txt</code>.
                  </p>
                  <p className="lyrics-note">
                    Marque as partes com um cabeçalho entre colchetes —{" "}
                    <code>[refrão]</code>, <code>[verso]</code>,{" "}
                    <code>[ponte]</code> — pra ligar os efeitos de intensidade.
              ...
              {hasLyrics && syncMode && (
                <footer className="sync-bar">
                  ...
                  <div className="sync-row sync-sections">
                    ...
                    {SECTION_ORDER.map((section) => (
                      <button
                        key={section}
                        className={
                          lyrics.lines[cursorIndex]?.section === section
                            ? "on"
                            : ""
                        }
                  ...
                  <div className="sync-row sync-io">
                    ...
                    <button onClick={() => setSyncCursor(0)}>
                      VOLTAR AO TOPO
                    </button>
                    ...
                    {projectLyrics && lyricsOrigin === "storage" && (
                      <button
                        onClick={useProjectLyrics}
                        title="Descarta o rascunho deste navegador"
                      >
      ...
      <main id="top">
        <section className="hero" aria-labelledby="track-title">
          ...
          <div className="hero-copy">
            <p className="eyebrow">
              <span /> Deftones / single experience
            </p>
      ...
      <div className="ticker" aria-hidden="true">
        <div>
          CHANGE · DEFTONES · 2000 · IN THE HOUSE OF FLIES ·&nbsp; CHANGE ·
          DEFTONES · 2000 · IN THE HOUSE OF FLIES ·&nbsp; CHANGE · DEFTONES ·
          2000 · IN THE HOUSE OF FLIES ·&nbsp; CHANGE · DEFTONES · 2000 · IN THE
          HOUSE OF FLIES ·&nbsp;
        </div>
      </div>
      ...
      <footer>
        ...
        <p>
          EXPERIÊNCIA NÃO OFICIAL FEITA POR FÃ.
          <br />O ÁUDIO TOCA LOCALMENTE, NO SEU APARELHO.
        </p>
